...
# This code creates a wedpages for all pairs, pf  how their extreme returns and swings are filtered by news. Plus it returns the list of Critical news. yhe list of "Critical news" generated  can be used in the actual bot to filter incoming critical news. One has to run news02 to create the list of critical DAYS, which are all the days when a critical news is reported and  is used in the optimization to check for the effects of removing days with crtiical news to be used as filter in the optimization of robots, as part of the money managmente, safity procedures.
#This code includes the days before, during and after the news, we found this was more effective at removing all swings and returns larger than 2% that just using the day the news were reporte. One losses for tradingdays, but plays safer.
...
#compile list of crtiical news by each pair...given both citical retruns and swings
    NameCriticalNewsReturns=c(Returns[[3]],Swings[[3]])
    if(!is_empty(NameCriticalNewsReturns)){
      NewsCritic=data.frame(Ticker=TickerX, NameCriticalNews=NameCriticalNewsReturns); 
      CriticalNewsByPair_Returns=rbind(CriticalNewsByPair_Returns, NewsCritic)}
    

##############list of news and days with critical swins only. Used to potentailly increase number of dates tradings
    #compile list of crtiical news by each pair...given both citical retruns and swings
    NameCriticalNewsSwings=c(Swings[[3]])
    if(!is_empty(NameCriticalNewsSwings)){
      NewsCritic=data.frame(Ticker=TickerX,NameCriticalNews=NameCriticalNewsSwings); 
      CriticalNewsByPair_SwinsOnly=rbind(CriticalNewsByPair_SwinsOnly, NewsCritic)}
    
      }

write.csv(CriticalNewsByPair_Returns, "News01_CriticalNews_ListByPair_ReturnsAndSwing.csv")


write.csv(CriticalNewsByPair_SwinsOnly, "News01_CriticalNews_ListByPair_SwingsOnly.csv")
