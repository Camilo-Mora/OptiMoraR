...
#news
  ...
  News$Date=format(as.Date(News$DateI,  format ="%b%d.%Y"), "%Y-%m-%d")
  
  News=News[order(as.Date(News$Date, format="%Y-%m-%d")),]
  
  
  News$X=NULL
  NewsIP= data.frame(Date=unique(News$Date)) #select unique dates to avoid multiple lines of news from the same date
  NewsIP$NewsId=1:nrow(NewsIP)
  News=merge(News,NewsIP, by="Date", all.x=TRUE)   #attach unique News ID to list of news

#Plots for corrections
Data=read.csv("D:/Scrips/Trading/ForexPairs2Months.csv")
...
#Attach news to dygraph price data
Paired$Date=substr(Paired$Date, 1, 10) #tickmark of dates in ohlc data
Paired= Paired %>% group_by(Date) %>% summarise(Tick = Tick [[1]]) #get first tickmark of date

News3=merge(Paired, NewsIP, by="Date", all=TRUE)
News3=News3[order(as.Date(News3$Date, format="%Y-%m-%d")),]
start <- which.min(is.na(News3$Tick))
News3=News3[start:nrow(News3),]
News3$NewID=na.locf(News3$Tick)
News3=News3[,c("NewsId", "NewID")]
colnames(News3)=c("NewsId","Tick")
News3=News3[complete.cases(News3),]

Dygraph=Dygraph %>% dyEvent(News3$Tick, News3$NewsId, labelLoc = "bottom",color ="lightcyan", strokePattern ="dashed")
News$Date=NULL
